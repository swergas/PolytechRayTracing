...
ScreenV2::ScreenV2(const Vector3& observer, const Vector3& aimedPoint,
...
bool ScreenV2::initFromWH3D(double width3D, double height3D) {
...
/*
bool ScreenV2::initFromDistScreen ( double distScreen, double ratioWH )
{
	double t_va = tan ( _viewAngle );
	if ( t_va < 0.05 && t_va > - 0.05 ) // TODO: find a proper epsilon
		return false;
...
bool ScreenV2::initFromDistScreen(double distScreen) {
	double t_va = tan(_viewAngle);
	if (t_va < 0.05 && t_va > -0.05) // TODO: find a proper epsilon
		return false;
	_distScreen = distScreen;
...
Vector3 ScreenV2::getPixel(double x, double y) { // TODO: maybe we should optimize our class data structure to ease this operation
